...
export class GetDealTimelineQuery {
  constructor(public readonly dealId: string) {}
}

@QueryHandler(GetDealTimelineQuery)
export class GetDealTimelineHandler implements IQueryHandler<GetDealTimelineQuery, any> {
  constructor(@Inject(DEAL_REPOSITORY) private readonly deals: DealRepository) {}

  async execute(query: GetDealTimelineQuery): Promise<any> {
